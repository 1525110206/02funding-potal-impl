package com.javaweb.funding.potal.controller;
...
import com.javaweb.funding.potal.listener.PassListener;
import com.javaweb.funding.potal.listener.RefuseListener;
...
public class MemberController {
	...
	public Object doUploadCert(HttpSession session, Data ds){
		...
		for(MemberCert memberCert : certimgs){
			...
			String nameone = UUID.randomUUID().toString() + extName;
			
			String filename = realPath + "/cert" + "/" + nameone;
			
			fileImg.transferTo(new File(filename));
			
			memberCert.setIconpath(nameone);
	...
	public String apply(HttpSession session){
		...
		if(ticket == null){
			...
			if("accttype".equals(pstep)){
				...
				//根据当前用户查询账户类型，然后根据账户类型查找需要上传的资源
				System.out.println(member.getAccttype());
